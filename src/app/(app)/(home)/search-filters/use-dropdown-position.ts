...
export const useDropdownPosition = (ref: RefObject<HTMLDivElement | null >|RefObject<HTMLDivElement>) => {
  const getDropdownPosition = () => {
    ...
    const dropdownWidth = 240; //Width of the dropdown, adjust as needed(w-60=15rem=240px)
    ...
    if (left + dropdownWidth > window.innerWidth) {
      //Align to right edge pf bottom instead
      left = rect.right + window.scrollX - dropdownWidth;
      //if still off-screen, align to right edge of the viewport with some padding
      if(left<0){
